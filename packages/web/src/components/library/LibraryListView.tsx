...
const ListRow = memo(function ListRow({ note }: { note: LiteratureNoteSummary }) {
  ...
  const typeLabel = useMemo(() => {
    if (!csl?.type) return note.sourceType.toUpperCase();
    ...
    return typeMap[csl.type] || csl.type;
  }, [csl?.type, note.sourceType]);
    ...
    >
      ...
      <div className="w-10 h-14 flex-shrink-0 rounded-md overflow-hidden bg-bg-deep relative">
        ...
        {note.progress > 0 && (
          <div className="absolute bottom-0 left-0 right-0">
            <ProgressIndicator progress={note.progress} height="h-0.5" animateOnMount={false} />
          </div>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="sm:hidden flex items-center gap-2 mt-0.5 text-xs text-text-secondary">
          ...
          {note.author && pubInfo && <span className="text-text-secondary/50">·</span>}
          {pubInfo && (
      ...
      <div className="hidden sm:flex items-center gap-3 flex-shrink-0">
        {/* Author column */}
        <div className="w-36 truncate" data-testid="list-row-author">
          {note.author ? (
            <span className="text-xs text-text-secondary">{note.author}</span>
          ) : (
